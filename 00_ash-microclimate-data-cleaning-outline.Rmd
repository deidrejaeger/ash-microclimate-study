...
Reproducible data cleaning steps
...
*Step 1:* import the treatment sensor datafiles
  ...
  - check that date and temp classes are correct: date_time: chr, tempF: num
...
*Step 4:* format the date and time in R friendly POSIXct format
*Step 5:* set factors for categorical name and serial number of sensors and ash removal status
*Step 6:* export a new csv with the cleaned data
*Step 7:* fill in a textfile called "readme-boulder-ash-data2018" with the metadata about the sensor data
...
>>>>>>> 383df3b4ed43f6ce53fdedb66f7e8b057ef85d44
...
# check the structure of how it is loaded
str(treatment_100e_after) # we will eventually want date to be formated as date
...
```{r step2-bind-into-1-csv}

# aggregate the before and after dataframes for all sensor data into 1 dataframe (best practice for tidy analyses)

# for example:
treatment100e_data <- rbind(treatment_100e_before, treatment_100e_after)

```
 
```{r step3-format-date-time}
str(treatment100e_data) # check that starting class for date is "character"
treatment100e_data <- treatment100e_data %>% 
  mutate(date_time = ymd_hms(date_time)) # format date in R-friendly format

str(treatment100e_data) # check that class structure changes to "POSIXct"
```
